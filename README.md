...
PRD 正文最前必须有「意图（AI 当前理解）」：说明要解决什么、准备交付什么、本轮不做什么、已确定的取舍和待确认问题。它是给后续执行者看的交接，不替代 RFC 的概述和动机。需求范围变化时先更新这节。PRD 还必须有解释需求的图，默认使用 Mermaid：主流程图说明使用过程；涉及模块、调用顺序、生命周期或迁移时，补关系图、时序图、状态图或前后对照。图放在相关正文旁，验证渲染和文字一致性，不另建 HTML PRD。
