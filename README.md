...
└── <slug>/
    ...
    ├── plan.md               # 跨 spec 的方案、决策和整套计划的评审状态
    ├── todo.md               # 索引,唯一进度真相源
...
**plan.md 和 spec 各管一层。** plan 只拥有跨 spec 的技术路线、已确认决策、完整约束/风险和验证基线；spec 只拥有一个纵向交付的范围、行为契约、验收及局部决策。spec 中摘出的约束是为实现提供的受控副本，plan 才是原始真相。

**计划评审是执行门禁。** 计划完成后，小需求由用户 review `plan.md + todo.md`，大需求由用户 review `plan.md + todo.md + 全部 specs`。整套计划只在 `plan.md` 维护一个「⏳ 待用户评审 / ✅ 已确认 · 可执行」状态，不逐个确认 spec。确认本身不会自动开始写代码，只有后续明确要求执行（或说「确认并开始 <spec>」）才会启动第一个 spec。

**归档要两个条件同时成立**:所有 todo 都完成,且用户主动开口。不会在特性做完时顺手归档,也不会主动提议 —— 归档是把东西移出视野,该由用户决定什么时候不想再看见它。
